...
def getSnapshot(ucdUrl, ucduserName, ucdPassword, appName, snapshotName) {
    println "**********************"
    ...
    wrap([$class: 'MaskPasswordsBuildWrapper', varPasswordPairs: [[password: "${ucdPassword}", var: 'ucdPassword']]]) {
    def udClient = "./udclient/udclient"
    def ucdCmd = "${udClient} -username ${ucduserName} -password ${ucdPassword} -weburl ${ucdUrl}"
    ...
    //echo "response is ${responseJson}"
    UDClient.mapFromJson(responseJson)
 }
}
}

def snapshotAlreadyExists(ucdUrl, ucduserName, ucdPassword, appName, snapshotName) {
    def alreadyExists = true
    try {
        getSnapshot(ucdUrl, ucduserName, ucdPassword, appName, snapshotName)
    } catch (Exception e) {
...
def deploy(ucdUrl, ucduserName, ucdPassword, requestJson) {
    println "**************************"
...
def getDeploymentStatus(ucdUrl, ucduserName, ucdPassword, requestId) {
    println "********************"
    println " Running UCD Status "
    println "********************"
    withEnv(['PATH+bin=/bin', 'PATH+usr=/usr/bin', 'PATH+local=/usr/local/bin', 'JAVA_HOME=/usr/lib/jvm/jre-1.7.0-openjdk.x86_64']){
...
def getRequestTrace(ucdUrl, ucduserName, ucdPassword, requestId) {
    println "***************************"
...
def getApplicationProcessUnfilledProperties(ucdUrl, ucduserName, ucdPassword, appName, processName) {
    println "**********************"
