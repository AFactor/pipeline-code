...
Created/modified on: $curr_date'"
			...
			if [ "$(grep $journey groups)" == "" ];then
				journey_id=$(curl -X GET --digest -u $GERRIT_HTTP_CREDS http://gerrit.sandbox.extranet.group/a/groups/ | sed 1d | /apps/tools/jq .\\"$journey\\"."id" | sed 's/"//g')
				journey_reviewer_id=$(curl -X GET --digest -u $GERRIT_HTTP_CREDS http://gerrit.sandbox.extranet.group/a/groups/ | sed 1d | /apps/tools/jq .\\"$journey-reviewer\\"."id" | sed 's/"//g')
				journey_admin_id=$(curl -X GET --digest -u $GERRIT_HTTP_CREDS http://gerrit.sandbox.extranet.group/a/groups/ | sed 1d | /apps/tools/jq .\\"$journey-admin\\"."id" |sed 's/"//g')
